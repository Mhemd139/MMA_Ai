...
COLORS = {
    ...
    "high guard": (255, 0, 0),
    "low guard": (0, 255, 255),
    "blocked_punch": (0, 0, 255),  # Red
    "landed_punch": (0, 255, 0)     # Green
}
...
def draw_counter(image, punches_blocked, punches_landed):
    ...
    cv2.rectangle(overlay, (10, 110), (350, 200), (0, 0, 0), -1)
    ...
    # Punches Blocked counter (Red)
    blocked_text = f"Punches Blocked: {punches_blocked}"
    cv2.putText(image, blocked_text, (20, 45), font, font_scale, (0, 0, 255), thickness, cv2.LINE_AA)
...
# === Main Detection Loop ===
def run_detection_with_huggingface():
    """Run detection with Hugging Face model integration"""
    print("🎬 Starting YOLO + Hugging Face detection...")
    ...
    model, processor = setup_huggingface_model()
    ...
    for frame_file in sorted(os.listdir(FRAME_DIR)):
        ...
        # Punch analysis with Hugging Face model
        punch_status = None
            ...
            for face_box in faces:
                if face_box and boxes_intersect(punch_box, face_box):
                    if model is not None:
                        ...
                        punch_region = image[punch_box[1]:punch_box[3], punch_box[0]:punch_box[2]]
                        if punch_region.size > 0:
                            # Convert to PIL and process
                            pil_image = Image.fromarray(cv2.cvtColor(punch_region, cv2.COLOR_BGR2RGB))
                            inputs = processor(pil_image, return_tensors="pt")
                            inputs = {k: v.to(device) for k, v in inputs.items()}
                            
                            with torch.no_grad():
                                outputs = model(**inputs)
                                model_predictions = torch.nn.functional.softmax(outputs.logits, dim=-1)
                                predicted_class = torch.argmax(model_predictions).item()
                                confidence = model_predictions[0][predicted_class].item()
    ...
    # Save metadata and create video
